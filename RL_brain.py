...
class DQNPrioritizedReplay:
    """
    This class DQNPrioritizedReplay is a modified version based on the original code from:
    ...
    def __init__(
        ...
        # self.replace_target_op = [tf.assign(t, (1 - self.theta) * t + self.theta * e)
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('eval_net'):
            c_names, n_l1, n_l2, w_initializer, b_initializer = \
                ['eval_net_params', tf.GraphKeys.GLOBAL_VARIABLES], 300, 150, \
    ...
    def learn(self):
        ...
        EXPLORATION_STOP = 60000
        LAMBDA = - (MAX_EPSILON - MIN_EPSILON) / EXPLORATION_STOP  # speed of decay
        ...
        reward = batch_memory[:, self.n_features + 1]  # after action !!!
        done = batch_memory[:, self.n_features + 2]  # done

        max_act4next = np.argmax(q_eval4next, axis=1)
        selected_q_next = q_next[batch_index, max_act4next]

        q_target[batch_index, eval_act_index] = reward + self.gamma * selected_q_next * (1 - done)  # double DQN
        ...
        self.epsilon = self.epsilon + LAMBDA if self.epsilon > MIN_EPSILON else MIN_EPSILON
        self.learn_step_counter += 1
        return self.epsilon, self.cost

    def save(self, path='./PR_DQN_model.ckpt'):
