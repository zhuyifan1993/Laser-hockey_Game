...
class LaserHockeyEnv(gym.Env, EzPickle):
    ...
    def __init__(self, mode = NORMAL):
        ...
        self.discrete_action_space = spaces.Discrete(8)
    ...
    def _compute_reward(self):
        ...
        hit = self.combo  #total hit in episode

        # r += reward_closeness_to_puck * 100
        # if self.puck.position[0] < CENTER_X:
        #     r += reward_puck_direction * reward_touch_puck * 10

        #r += reward_closeness_to_puck * 1 + reward_puck_direction * 1

        if self.done:
            if self.winner == 0: # tie
                if hit == 0: # means agent does nothing
                    r -= 2
                else:
                    r += 0 # hit the ball and tie
            elif self.winner == 1: # you won
                if hit == 0: # the opponent shots himself
                    r = 0
            else: # you lose
                r -= 2
        return r
    ...
    def discrete_to_continous_action(self, discrete_action):
        ...
        Action 6: 1 in angle
        '''
        action_cont = [(discrete_action==1) * -0.65 + (discrete_action==2) * 0.65, # player x
                       (discrete_action==3) * -0.65 + (discrete_action==4) * 0.65, # player y
                       (discrete_action==5) * -0.65 + (discrete_action==6) * 0.65] # player angle
        ...
        if discrete_action == 0:  # move forward
            return [1, 0, 1]
        elif discrete_action == 1:# move back
            return [-1, 0, 1]
        elif discrete_action == 2:# move forward and up
            return [1, 1, 1]
        elif discrete_action == 3:# move forward and down
            return [1, -1, 1]
        elif discrete_action == 4:# move up
            return [0, 1, 1]
        elif discrete_action == 5:# move down
            return [0, -1, 1]
        elif discrete_action == 6:# move back and up
            return [-1, 1, 1]
        elif discrete_action == 7:# move back and down
            return [-1, -1, 1]
